...
* Fix a 1.10.2 regression where `run_query()` on statements that return no column metadata over HTTP (e.g. `GRANT ... ON CLUSTER`) failed with `zip() argument 2 is longer than argument 1` ([#XXX](https://github.com/ClickHouse/dbt-clickhouse/pull/XXX)).
