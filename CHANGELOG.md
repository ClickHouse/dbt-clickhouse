...
#### Improvements
* Add an optional `external_id` argument to `clickhouse_s3source()`, settable per model or in the `vars` S3 configuration alongside `role_arn`, and emitted as `extra_credentials(role_arn='...', external_id='...')`. ClickHouse Cloud forwards it as the `ExternalId` parameter of the AWS STS `AssumeRole` call, so the assumed role's trust policy can require a shared secret through an `sts:ExternalId` condition. Specifying `external_id` without `role_arn` raises an error rather than silently dropping it ([#698](https://github.com/ClickHouse/dbt-clickhouse/pull/698)).
...
* Refresh parameters of refreshable materialized views (`interval`, `randomize`, `depends_on`) are now updated in place on every `dbt run` using `ALTER TABLE ... MODIFY REFRESH`, the same way the MV query is updated via `MODIFY QUERY`. Previously, changing them required `dbt run --full-refresh`, which dropped and recreated the MV. Changes that ClickHouse cannot apply in place — converting a regular MV to a refreshable one (or back), and toggling the `append` setting — now fail with a clear error asking for a `--full-refresh` run instead of being silently ignored ([#611](https://github.com/ClickHouse/dbt-clickhouse/issues/611), [#688](https://github.com/ClickHouse/dbt-clickhouse/pull/688)).
...
* Contract validation and schema-change introspection now apply model `query_settings` (e.g. `join_use_nulls`) when deriving the query schema, so contracts match runtime column nullability ([#667](https://github.com/ClickHouse/dbt-clickhouse/issues/667), [#689](https://github.com/ClickHouse/dbt-clickhouse/pull/689)).
* Materialized view targets with `catchup=false` now create configured projections and indexes. Previously only the catchup=true path called `add_index_and_projections` ([#690](https://github.com/ClickHouse/dbt-clickhouse/pull/690)).
