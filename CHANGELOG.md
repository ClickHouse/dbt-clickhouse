...
* Add a `reuse_connections` profile option (default `true`, preserving current behavior). When set to `false`, dbt closes the underlying connection at the end of each model so the next model opens a fresh one — useful for multi-replica ClickHouse Cloud services where the load balancer routes by TCP connection and a long-lived pool would otherwise pin a `dbt run` to a single replica ([#669](https://github.com/ClickHouse/dbt-clickhouse/issues/669)).
