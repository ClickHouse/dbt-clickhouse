...
* Apply column-level `codec` and `ttl` to distributed models on create and on add/modify column. Storage clauses now target the shard-local table instead of the Distributed proxy ([#741](https://github.com/ClickHouse/dbt-clickhouse/pull/741)).
...
#### Improvements
* Update refreshable materialized view parameters (`interval`, `randomize`, `depends_on`) in place on `dbt run` via `MODIFY REFRESH`. Converting between regular and refreshable MVs, or toggling `append`, still requires `--full-refresh` ([#688](https://github.com/ClickHouse/dbt-clickhouse/pull/688)).
...
* Add a `server_host_name` profile option so TLS certificate verification can use a different hostname than the connection target ([#687](https://github.com/ClickHouse/dbt-clickhouse/pull/687)).
* Add a `reuse_connections` profile option (default `true`). When `false`, dbt opens a fresh connection per model so multi-replica clusters can load-balance connections across replicas ([#670](https://github.com/ClickHouse/dbt-clickhouse/pull/670)), ([#686](https://github.com/ClickHouse/dbt-clickhouse/pull/686)).
* Add an `index` parameter to the `projections` model config as syntax sugar for `_part_offset` index projections ([#651](https://github.com/ClickHouse/dbt-clickhouse/pull/651)).
...
* Allow clustered profiles without `database_engine` ([#697](https://github.com/ClickHouse/dbt-clickhouse/pull/697)).
* Fix S3 table function calls that set `compression` without `structure` ([#695](https://github.com/ClickHouse/dbt-clickhouse/pull/695)).
* Fix snapshots failing with `TABLE_ALREADY_EXISTS` on an orphaned `<snapshot>__dbt_tmp` after an interrupted run ([#692](https://github.com/ClickHouse/dbt-clickhouse/pull/692)).
* Fall back to `DROP` + `CREATE` when replacing views on filesystems that do not support atomic rename ([#684](https://github.com/ClickHouse/dbt-clickhouse/pull/684)).
* Fix `insert_overwrite` failing on ClickHouse 26.6+ when replacing a partition from an empty source ([#680](https://github.com/ClickHouse/dbt-clickhouse/pull/680)).
* Fix multiple materialized views breaking when a `--` comment precedes the first marker ([#683](https://github.com/ClickHouse/dbt-clickhouse/pull/683)).
* Apply model `query_settings` during contract validation and schema-change introspection ([#689](https://github.com/ClickHouse/dbt-clickhouse/pull/689)).
* Create configured projections and indexes on materialized view targets with `catchup=false` ([#690](https://github.com/ClickHouse/dbt-clickhouse/pull/690)).
