...
**Next steps**: `ON CLUSTER` DDL and the distributed materializations for self-managed clusters, grants, `dbt clone`, `dbt source freshness` from table metadata, fixes for the remaining bugs and passing the full v1 test suite to ensure full parity.
...
The tables below compare the features available in `dbt-clickhouse` (v1) with their status in the dbt Core v2 adapter. Every feature listed is supported in v1 unless the notes say otherwise; the status column is for dbt Core v2.
