...
{% macro clickhouse__list_relations_without_caching(schema_relation) %}
  {% call statement('list_relations_without_caching', fetch_result=True) -%}
    with mv_sources as (
      -- Find all MVs and their target tables (database and name of the MV)
      select
        name as mv_name,
    ...
    select
      ...
      groupUniqArrayIf(concat(mv_sources.mv_database, '.', mv_sources.mv_name), mv_sources.mv_name != '') as mvs_pointing_to_it,
