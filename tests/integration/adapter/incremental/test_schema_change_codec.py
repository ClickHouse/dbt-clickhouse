...
class TestSchemaChangeWithCodec:
    ...
    def models(self):
        return {
            "schema_change_codec_append.sql": schema_change_with_codec_sql % ("incremental", "append_new_columns"),
            "schema_change_codec_distributed_append.sql": schema_change_with_codec_sql % ("distributed_incremental", "append_new_columns"),
            "schema.yml": schema_change_with_codec_yml
        }

    @pytest.mark.parametrize("model", ("schema_change_codec_append", "schema_change_codec_distributed_append"))
...
class TestSyncAllColumnsWithCodec:
    ...
    def models(self):
        return {
            ...
            "sync_codec_distributed_test.sql": sync_all_columns_with_codec_sql % "distributed_incremental",
    ...
    def test_sync_all_columns_with_codec(self, project, model):
        ...
        result_types = project.run_sql(f"select toColumnTypeName(col_1) from {model} limit 1", fetch="one")
